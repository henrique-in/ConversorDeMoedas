...
constructor(props){
 ...
 async converter(){
  ...
  const response = await api.get(`convert?q=${de_para}&compact=ultra&apiKey=8fcb1953bfb24c184526`)
  let cotacao = response.data[de_para];

  let resultado = (cotacao * parseFloat(this.state.moedaB_valor))
  
  this.setState({
    valorfinal: resultado.toFixed(2)
  });
  ...
  render(){
    ...
    return(
      ...
      <View style={styles.container}>
        ...
        { (this.state.valorfinal === 0 ? '' : 'R$' + this.state.valorfinal)}
        </Text>
